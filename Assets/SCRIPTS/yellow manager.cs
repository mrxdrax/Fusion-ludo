...
public class yellowmanager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (yellow_goti == null)
        {
            Debug.LogError("Yellow goti prefab not assigned!");
            return;
        }
        if (spawn_area == null || spawn_area.Length == 0)
        {
            Debug.LogError("Yellow spawn area not assigned!");
            return;
        }
        if (yellow_path == null || yellow_path.Length == 0)
        {
            Debug.LogError("Yellow path not assigned!");
            return;
        }
    ...
    /// <summary>
    /// Spawn all gotis for yellow player
    ...
    public void Spawn()
    {
        for (int i = 0; i < spawn_area.Length; i++)
        {
            ...
            GameObject gotiInstance = Instantiate(yellow_goti, spawn_area[i].transform.position, Quaternion.identity);
