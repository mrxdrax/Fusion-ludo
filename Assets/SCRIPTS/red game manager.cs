...
public class redgamemanager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (red_goti == null)
        {
            Debug.LogError("Red goti prefab not assigned!");
            return;
        }
        if (spawn_area == null || spawn_area.Length == 0)
        {
            Debug.LogError("Red spawn area not assigned!");
            return;
        }
        if (red_path == null || red_path.Length == 0)
        {
            Debug.LogError("Red path not assigned!");
            return;
        }
    ...
    /// <summary>
    /// Spawn all gotis for red player
    ...
    public void Spawn()
    {
        for (int i = 0; i < spawn_area.Length; i++)
        {
            ...
            GameObject gotiInstance = Instantiate(red_goti, spawn_area[i].transform.position, Quaternion.identity);
